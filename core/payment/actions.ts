...
import { getSubscription } from "@/core/payment/queries";
...
export const updateSubscription = async (planId: string) => {
  ...
  const currentPlan = config.stripe.plans.find(
    (plan) => plan.id === subscription.planId
  );
  const newPlan = config.stripe.plans.find((plan) => plan.id === planId);
  ...
  if (currentPlan?.includedQuota > newPlan.includedQuota) {
